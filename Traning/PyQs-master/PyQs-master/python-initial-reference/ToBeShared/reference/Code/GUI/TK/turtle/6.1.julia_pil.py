...
import random 
C_index = random.randint(0, len(C_S)-1)
print(len(C_S), C_index, end=" ")
C = C_S[C_index]
print(C)
...
def draw(ax,center_x,center_y, mag, maxIt , width=width, height=height):
    ...
    st = time.time()    
    pixels  = julia_set2(C, xmin,xmax,ymin,ymax,width,height,maxIt)    
    #pixels  = julia_set4(C, xmin,xmax,ymin,ymax,width,height,maxIt)
    print("-"*40)
