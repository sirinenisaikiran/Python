...
class Framework(Frame):
    def __init__(self, parent, h, x=-0.75, y=0, m=1.5, iterations=None, imgWidth=None, imgHeight=None, save=False, multi=True, opt1=False, opt2=False):
        ...
        self.fractal = Mandelbrot(self.canvasW, self.canvasH, x=x, y=y, m=m, iterations=iterations, w=imgWidth, h=imgHeight, multi=multi, opt1=opt1, opt2=opt2)
        self.setPalette()
        self.pixelColors = []
        self.img = None
        self.save = save
        self.draw()      
        #parent.bind("<Button-1>", self.simulate)
...
def main():
    ...
    parser.add_argument('-o1', '--Opt1', action='store_true', help="Optimizing 1")
    parser.add_argument('-o2', '--Opt2', action='store_true', help="Optimizing 2")
    ...
    if None not in [args.x, args.y, args.magnification]:
        render = Framework(master, height, x=args.x, y=args.y, m=args.magnification, multi=args.noMulti,
                           iterations=args.iterations, imgWidth=args.width, imgHeight=args.height, save=args.save, opt1=args.Opt1, opt2=args.Opt2)
    else:
        ...
        render = Framework(master, height, multi=args.noMulti, iterations=args.iterations,
                           imgWidth=args.width, imgHeight=args.height, save=args.save, opt1=args.Opt1, opt2=args.Opt2)
